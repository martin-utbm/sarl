...
public class CsharpExpressionGenerator extends AbstractExpressionGenerator {
	...
	/** Constructor.
	 *
	 * @param keywordProvider the provider of C# keywords. ->We also use CsharpElements static values
	 */
	@Inject
	public CsharpExpressionGenerator(CsharpKeywordProvider keywordProvider) {
		super(keywordProvider);
	}

	@Override
	protected IExtraLanguageConversionInitializer getTypeConverterInitializer() {
		return CsharpInitializers.getTypeConverterInitializer();
	}

	@Override
	protected IExtraLanguageConversionInitializer getFeatureNameConverterInitializer() {
		return CsharpInitializers.getFeatureNameConverterInitializer();
	}
	...
	protected void generateClosureDefinition(XClosure closure, IAppendable it, IExtraLanguageGeneratorContext context) {
		if (!it.hasName(closure)) {
			...
			.append(closureType).append(",").append(objType.getType()).append(") ").append(CsharpElements.CSHARP_SCOPE_OPENING) //$NON-NLS-1$ //$NON-NLS-2$
			.increaseIndentation().newLine().append("") //$NON-NLS-1$
	...
	protected void generateAnonymousClassDefinition(AnonymousClass anonClass, IAppendable it, IExtraLanguageGeneratorContext context) {
		if (!it.hasName(anonClass) && it instanceof CsharpAppendable) {
			...
			assert rootGenerator instanceof CsharpGenerator;
			...
			((CsharpGenerator) rootGenerator).generateTypeDeclaration(anonClass,
	...
	protected XExpression _generate(XBlockExpression block, IAppendable it, IExtraLanguageGeneratorContext context) {
		...
		it.append(CsharpElements.CSHARP_ENDSTATEMENT);//TODO : factorize this
	...
	protected XExpression _generate(XNumberLiteral literal, IAppendable it, IExtraLanguageGeneratorContext context) {
		appendReturnIfExpectedReturnedExpression(it, context);//100_00 !
	...
	protected XExpression _generate(XStringLiteral literal, IAppendable it, IExtraLanguageGeneratorContext context) {
		appendReturnIfExpectedReturnedExpression(it, context);
		//TODO : maybe force unicode format here
		it.append("\"").append(Strings.convertToJavaString(literal.getValue())).append("\""); //$NON-NLS-1$//$NON-NLS-2$
		return literal;
	}
	...
	protected XExpression _generate(XBinaryOperation operation, IAppendable it, IExtraLanguageGeneratorContext context) {
		...
		if (operator != null) {
			...
			//we should prepend the inverse condition sign (!)
			if(operator=="!=")
			{
				it.append("!"); //$NON-NLS-1$
			}
			...
			case "!==": //$NON-NLS-1$
				...
				break;
			default:
				throw new IllegalArgumentException(MessageFormat.format(Messages.CsharpExpressionGenerator_0, operator));
			}
	...
	protected XExpression _generate(XNullLiteral literal, IAppendable it, IExtraLanguageGeneratorContext context) {
		appendReturnIfExpectedReturnedExpression(it, context);
		it.append(CsharpElements.CSHARP_NULL_REFERENCE); //$NON-NLS-1$
		return literal;
	}
	...
	protected XExpression _generate(XSwitchExpression switchStatement, IAppendable it, IExtraLanguageGeneratorContext context) {
		...
		for (final XCasePart caseExpression : switchStatement.getCases()) {
			if (fallThrough) {
				...
				it.append("else if ("); //$NON-NLS-1$
			}
			if (caseExpression.getTypeGuard() != null) {
				
				it.append(varName);
				it.append("==typeof("); //$NON-NLS-1$
				it.append(caseExpression.getTypeGuard().getType());
				it.append(")"); //$NON-NLS-1$
	...
	protected static CsharpAppendable appendEndStatement(CsharpAppendable it)
	{
		it.append(CsharpElements.CSHARP_ENDSTATEMENT).newLine();
		return it;
	}
	...
	public static String toDefaultValue(JvmTypeReference type) {
		...
		return CsharpElements.CSHARP_NULL_REFERENCE; //$NON-NLS-1$
	}
	...
	public class PyFeatureCallGenerator extends FeatureCallGenerator {
			...
			if (elements != null && !elements.isEmpty()) {
				...
				for (final Object element : elements) {
					...
					if (element instanceof XExpression) {
						CsharpExpressionGenerator.this.generate((XExpression) element, this.codeReceiver, this.context);
		...
		protected void appendCall(JvmIdentifiableElement calledFeature, List<Object> leftOperand,
			...
			if (beginOfBlock != null) {
				...
				CsharpExpressionGenerator.this.generate(beginOfBlock.apply(), this.codeReceiver, this.context);
				this.codeReceiver.append(" != null)"); //$NON-NLS-1$
			...
			if (args != null) {
				...
				for (final XExpression arg : args) {
					...
					CsharpExpressionGenerator.this.generate(arg, this.codeReceiver, this.context);
				}
				this.codeReceiver.append(")"); //$NON-NLS-1$
			} else {
				this.codeReceiver.append(name);
